...
public class jTPCCTData {
	...
	public static AtomicLong taskId = new AtomicLong(0L); // #RAIN
	...
	private boolean printSQL = false; // #RAIN

	private int currentOOid = -1; // #RAIN
	private int reservedOOid = -1; // #RAIN
	private long commitTS = 1; // #RAIN
	private long rrts = 0; // #RAIN
	private long rwts = 0; // #RAIN
	private long wrts = 0; // #RAIN
	...
	// #RAIN
	public void setTerminalId(int termId) {
		terminalId = termId;
	}
	...
	public int getReservedOOID() {
		return reservedOOid;
	}

	// #RAIN：解析SQL语句参数并替换
	public static String renderSQL(String sql, Object... params) {
		for (Object param : params) {
			String val;
			if (param == null) {
				val = "NULL";
			} else if (param instanceof String || param instanceof java.sql.Date || param instanceof java.sql.Timestamp) {
				val = "'" + param.toString().replace("'", "''") + "'";
			} else {
				val = param.toString();
			}
	...
	// #RAIN：记录日志
	public static synchronized void log(String txnType, String sql, Object... params) {
		String fullSQL = renderSQL(sql, params);
		long currentTaskId = taskId.get();
		try (FileWriter fw = new FileWriter("/data/executed_sql.csv", true)) {
			fw.write(txnType + "," + currentTaskId + ",\"" + fullSQL.replaceAll("\n", " ") + "\"\n");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}

	public void execute(Logger log, jTPCCConnection db, jTPCC parent) throws Exception {
		...
		switch (transType) {
			case TT_NEW_ORDER:
				executeNewOrder(log, db, parent);
				break;

			case TT_PAYMENT:
				executePayment(log, db, parent);
				break;
		...
		jTPCC.latencyRecords.get(jTPCCTData.transTypeNames[transType]).add(transEnd - transStart);
	}
	...
	public void generateNewOrder(Logger log, jTPCCRandom rnd, long due, int ooid) {
		...
		while (i < o_ol_cnt) // 2.4.1.5
		{
			...
			newOrder.ol_quantity[i] = jTPCC.csv.orderline.ol_quantity.get(ooid);
			i++;
		}
	...
	private void executeNewOrder(Logger log, jTPCCConnection db, jTPCC parent) throws Exception {
		...
		jTPCCDataitem dataitem_selectStock = new jTPCCDataitem();
		jTPCCDataitem dataitem_updateStock = new jTPCCDataitem();
		...
		try {
			...
			jTPCCTransaction current_transaction = new jTPCCTransaction(currentOOid);

			parent.addTransactionToRunningPool(terminalId, current_transaction);
			...
			if (jTPCC.concurrencyControl.equals("mogi")) {
				jTPCCDataitem dataitem = new jTPCCDataitem(1, 1, newOrder.w_id, newOrder.d_id, 0, 0);

				if (parent.addDataitemToTransaction(terminalId, dataitem)) {
			...
			// #RAIN
			if (printSQL) {
				log(transTypeNames[transType],
						"SELECT d_tax, d_next_o_id FROM bmsql_district WHERE d_w_id = ? AND d_id = ?",
						newOrder.w_id,
						newOrder.d_id);
			}
			...
			for (int i = 0; i < 8; i++) {
				...
				if (jTPCC.concurrencyControl.equals("mogi")) {
					jTPCCDataitem dataitem = new jTPCCDataitem(1, 2, newOrder.w_id, newOrder.d_id, newOrder.c_id, 0);
			...
			for (int i = 0; i < ol_cnt; i++) {
				...
				if (jTPCC.concurrencyControl.equals("mogi")) {
					jTPCCDataitem dataitem = new jTPCCDataitem(1, 3, 0, 0, 0, newOrder.ol_i_id[seq]);
				...
				dataitem_selectStock = new jTPCCDataitem(1, 4, newOrder.ol_supply_w_id[seq], 0, 0, newOrder.ol_i_id[seq]);

				if (parent.needsConflictDetection() && !jTPCC.concurrencyControl.equals("others")) {
					if (parent.findCurrentLog(dataitem_selectStock)) {
						// parent.addErrorNum();
						throw new SQLException();
					}

					parent.addCurrentLog(currentOOid, dataitem_selectStock);
				}

				if (jTPCC.concurrencyControl.equals("tictoc")) {
					if (parent.dataTimestamps.containDataKey(dataitem_selectStock)) {
						jTPCCDataTS.DataValue value = parent.dataTimestamps.getData(dataitem_selectStock);

						if (value != null) {
							synchronized (parent.dataTimestamps.getData(dataitem_selectStock).getLock()) {
								parent.dataTimestamps.addRTS(dataitem_selectStock, 1);

								if (commitTS < parent.dataTimestamps.getData(dataitem_selectStock).getWTS()) {
									commitTS = parent.dataTimestamps.getData(dataitem_selectStock).getWTS();
								}
							}
						}
					} else {
						parent.dataTimestamps.storeData(dataitem_selectStock, 1, 0);

						if (commitTS < parent.dataTimestamps.getData(dataitem_selectStock).getWTS()) {
							commitTS = parent.dataTimestamps.getData(dataitem_selectStock).getWTS();
						}
					}
				} else if (jTPCC.concurrencyControl.equals("mogi")) {
					if (parent.addDataitemToTransaction(terminalId, dataitem_selectStock)) {
						throw new SQLException("Timeout");
					}
				}

				if (jTPCC.concurrencyControl.equals("calvin") || jTPCC.concurrencyControl.equals("s2pl")) {
					long lockStart = System.currentTimeMillis();
					parent.lockIID(currentOOid);
					long lockEnd = System.currentTimeMillis();
					parent.addToLockTime(lockEnd - lockStart + 1);
				}

				// #RAIN
				if (printSQL) {
					log(transTypeNames[transType],
							"SELECT s_quantity, s_data, s_dist_01, s_dist_02, s_dist_03, s_dist_04, s_dist_05, s_dist_06, s_dist_07, s_dist_08, s_dist_09, s_dist_10 FROM bmsql_stock WHERE s_w_id = ? AND s_i_id = ?",
				...
				if (parent.needsLogAnalysis() && dataitem_selectStock.getTableId() != 0)
					parent.addTransactionLog(currentOOid, dataitem_selectStock);
				...
				total_amount += newOrder.ol_amount[seq] * (1.0 - newOrder.c_discount) * (1.0 + newOrder.w_tax + newOrder.d_tax);
				...
				dataitem_updateStock = new jTPCCDataitem(2, 4, newOrder.ol_supply_w_id[seq], 0, 0, newOrder.ol_i_id[seq]);

				if (parent.needsConflictDetection() && !jTPCC.concurrencyControl.equals("others")) {
					parent.removeCurrentLog(currentOOid, true);

					if (parent.findCurrentLog(dataitem_updateStock)) {
						throw new SQLException("Conflict Detected");
					}

					parent.addCurrentLog(currentOOid, dataitem_updateStock);
				}
				...
				if (jTPCC.concurrencyControl.equals("tictoc")) {
					if (parent.dataTimestamps.containDataKey(dataitem_selectStock)) {
						...
						if (value != null) {
							synchronized (parent.dataTimestamps.getData(dataitem_selectStock).getLock()) {
								wrts = parent.dataTimestamps.getData(dataitem_selectStock).getRTS();

								parent.dataTimestamps.setWTS(dataitem_selectStock, wrts + 1);
								parent.dataTimestamps.setRTS(dataitem_selectStock, wrts + 1);
					...
					if (parent.addDataitemToTransaction(terminalId, dataitem_updateStock)) {
						throw new SQLException("Timeout");
					}
				}
				...
				if (printSQL) {
					log(transTypeNames[transType],
							"UPDATE bmsql_stock SET s_quantity = 10, s_ytd = s_ytd + 5, s_order_cnt = s_order_cnt + 1, s_remote_cnt = s_remote_cnt + 1 WHERE s_w_id = ? AND s_i_id = ?",
							newOrder.ol_supply_w_id[seq],
							newOrder.ol_i_id[seq]);
				}
			...
			if (parent.needsLogAnalysis() && dataitem_updateStock.getTableId() != 0)
				parent.addTransactionLog(currentOOid, dataitem_updateStock);
			...
			if (jTPCC.concurrencyControl.equals("tictoc")) {
				...
				synchronized (parent.dataTimestamps.getData(dataitem_selectStock).getLock()) {
					rrts = parent.dataTimestamps.getData(dataitem_selectStock).getRTS();
					rwts = parent.dataTimestamps.getData(dataitem_selectStock).getWTS();
					wrts = parent.dataTimestamps.getData(dataitem_selectStock).getRTS();

					if (commitTS >= wrts && commitTS >= rwts && commitTS <= rrts) {
						reservedOOid = -1;
			...
			if (parent.needsConflictDetection() && !jTPCC.concurrencyControl.equals("others")) {
				parent.removeCurrentLog(currentOOid, false);
			}

			if (jTPCC.concurrencyControl.equals("calvin") || jTPCC.concurrencyControl.equals("s2pl")) {
				parent.unlockIID(currentOOid);
			} else if (jTPCC.concurrencyControl.equals("mogi")) {
				parent.commitTxnInMogi(terminalId);
			}
			...
			try {
				...
				if (!jTPCC.concurrencyControl.equals("aria") && !parent.shouldRetry()) {
					parent.addAbortNum();
				}

				if (parent.needsLogAnalysis()) {
					parent.removeTransactionLog(currentOOid);
				}
			...
			try {
				...
				if (!jTPCC.concurrencyControl.equals("aria") && !parent.shouldRetry()) {
					...
					// parent.addToAbortTime(20);
					// 若事务中止，且不需要重试，则设置20ms的延迟
					// try {
					// Thread.sleep(20);
	...
	private void executePayment(Logger log, jTPCCConnection db, jTPCC parent) throws Exception {
		PreparedStatement stmt;
		ResultSet rs;
		...
		try {
			...
			if (jTPCC.concurrencyControl.equals("mogi")) {
				jTPCCDataitem dataitem = new jTPCCDataitem(2, 1, payment.w_id, payment.d_id, 0, 0);
			...
			if (jTPCC.concurrencyControl.equals("mogi")) {
				jTPCCDataitem dataitem = new jTPCCDataitem(1, 1, payment.w_id, payment.d_id, 0, 0);
			...
			if (jTPCC.concurrencyControl.equals("mogi")) {
				jTPCCDataitem dataitem = new jTPCCDataitem(2, 5, payment.w_id, payment.d_id, 0, 0);
			...
			if (jTPCC.concurrencyControl.equals("mogi")) {
				jTPCCDataitem dataitem = new jTPCCDataitem(1, 5, payment.w_id, 0, 0, 0);
			...
			if (printSQL) {
				log(transTypeNames[transType],
						"SELECT c_first, c_middle, c_last, c_street_1, c_street_2, c_city, c_state, c_zip, c_phone, c_since, c_credit, c_credit_lim, c_discount, c_balance FROM bmsql_customer WHERE c_w_id = ? AND c_d_id = ? AND c_id = ?",
						payment.c_w_id,
						payment.c_d_id,
						payment.c_id);
			}
			...
			if (jTPCC.concurrencyControl.equals("mogi")) {
				jTPCCDataitem dataitem = new jTPCCDataitem(1, 2, payment.c_w_id, payment.c_d_id, payment.c_id, 0);
			...
			if (jTPCC.concurrencyControl.equals("mogi")) {
				jTPCCDataitem dataitem1 = new jTPCCDataitem(1, 2, payment.c_w_id, payment.c_d_id, payment.c_id, 0);
				jTPCCDataitem dataitem2 = new jTPCCDataitem(2, 2, payment.c_w_id, payment.c_d_id, payment.c_id, 0);

				if (parent.addDataitemToTransaction(terminalId, dataitem1)) {
					throw new SQLException("Timeout");
				}

				if (parent.addDataitemToTransaction(terminalId, dataitem2)) {
					throw new SQLException("Timeout");
				}
			}
