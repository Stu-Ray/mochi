...
public class CoreWorkload extends Workload {
  ...
  // #RAIN
  protected int opsPerTxn = 1;
  protected long txnSeed = -1;
  protected Random txnRandom = null;
  protected static AtomicLong txnCounter = new AtomicLong(0);
  ...
  // #RAIN ：新增函数
  protected static NumberGenerator getFieldLengthGenerator(Properties p, Random rand) throws WorkloadException {
    NumberGenerator fieldlengthgenerator;
    String fieldlengthdistribution = p.getProperty(
        FIELD_LENGTH_DISTRIBUTION_PROPERTY, FIELD_LENGTH_DISTRIBUTION_PROPERTY_DEFAULT);
    int fieldlength = Integer.parseInt(p.getProperty(FIELD_LENGTH_PROPERTY, FIELD_LENGTH_PROPERTY_DEFAULT));
    ...
    if (fieldlengthdistribution.compareTo("constant") == 0) {
      ...
      fieldlengthgenerator = new UniformLongGenerator(minfieldlength, fieldlength, rand);
    } else if (fieldlengthdistribution.compareTo("zipfian") == 0) {
      fieldlengthgenerator = new ZipfianGenerator(minfieldlength, fieldlength, rand);
    } else if (fieldlengthdistribution.compareTo("histogram") == 0) {
      try {
        fieldlengthgenerator = new HistogramGenerator(fieldlengthhistogram);
      } catch (IOException e) {
  ...
  public void init(Properties p) throws WorkloadException {
    // -------------------------------- #RAIN --------------------------------
    opsPerTxn = Integer.parseInt(p.getProperty("operationspertransaction", "1")); // 每个事务包含多少操作
    txnSeed = Long.parseLong(p.getProperty("txnseed", "-1")); // 可复现随机数种子
    if (txnSeed != -1) {
      txnRandom = new Random(txnSeed);
    }
    // 随机种子
    Random seed1 = (txnSeed == -1) ? new Random() : new Random(txnSeed);
    Random seed2 = (txnSeed == -1) ? new Random() : new Random(txnSeed * 3 + 1);
    Random seed3 = (txnSeed == -1) ? new Random() : new Random(txnSeed * 5 + 3);
    Random seed4 = (txnSeed == -1) ? new Random() : new Random(txnSeed * 7 + 5);
    // ----------------------------- #RAIN END --------------------------------
    ...
    fieldlengthgenerator = CoreWorkload.getFieldLengthGenerator(p, seed1); // #RAIN
    ...
    operationchooser = createOperationGenerator(p, seed2); // #RAIN

    transactioninsertkeysequence = new AcknowledgedCounterGenerator(recordcount);
    if (requestdistrib.compareTo("uniform") == 0) {
      keychooser = new UniformLongGenerator(insertstart, insertstart + insertcount - 1, seed3); // #RAIN
      ...
      keychooser = new ScrambledZipfianGenerator(insertstart, insertstart + insertcount + expectednewkeys, seed3); // #RAIN
    ...
    fieldchooser = new UniformLongGenerator(0, fieldcount - 1, seed4); // #RAIN
  ...
  public boolean doTransaction(DB db, Object threadstate) {
    ...
    Random txnRandom = (txnSeed == -1) ? new Random() : new Random(txnSeed + txnId);

    for (int i = 0; i < opsPerTxn; i++) {
      String operation;
      if (txnRandom != null) {
        operation = operationchooser.nextValue(txnRandom);
      } else {
  ...
  // #RAIN : 新增函数
  protected static DiscreteGenerator createOperationGenerator(final Properties p, Random rand) {
    if (p == null) {
      throw new IllegalArgumentException("Properties object cannot be null");
    }
    final double readproportion = Double.parseDouble(
    ...
    final DiscreteGenerator operationchooser = new DiscreteGenerator(rand);
    if (readproportion > 0) {
      operationchooser.addValue(readproportion, "READ");
    }
