 ...
 * itemspace. Use this, instead of
 * @ZipfianGenerator, if you don't want the head of the distribution (the
 * popular items) clustered together.
 */
public class ScrambledZipfianGenerator extends NumberGenerator {
   ...
   * we have only precomputed zeta for one
   * zipfian constant
   ...
   * specified zipfian constant. If you
   * use a zipfian constant other than 0.99, this will take a long time to
   * complete because we need to recompute zeta.
   *
   * @param min             The smallest integer to generate in the sequence.
   * @param max             The largest integer to generate in the sequence.
  ...
  // #RAIN : 新增构造函数
  public ScrambledZipfianGenerator(long items, Random rand) {
    this(0, items - 1, rand);
  }

  // #RAIN : 新增构造函数
  public ScrambledZipfianGenerator(long min, long max, Random rand) {
    this(min, max, ZipfianGenerator.ZIPFIAN_CONSTANT, rand);
  }

  // #RAIN : 新增构造函数
  public ScrambledZipfianGenerator(long min, long max, double zipfianconstant, Random rand) {
    this.min = min;
    this.max = max;
    this.rand = rand;
    itemcount = this.max - this.min + 1;
    if (zipfianconstant == USED_ZIPFIAN_CONSTANT) {
      gen = new ZipfianGenerator(0, ITEM_COUNT, zipfianconstant, ZETAN, rand);
    } else {
      gen = new ZipfianGenerator(0, ITEM_COUNT, zipfianconstant, rand);
    }
  }
