...
public class jTPCCTerminal implements jTPCCConfig, Runnable
{
	...
	{	
		...
		{
			try {
				Thread.sleep(20);	// Calvin需要预先读快照，因此设置延迟20毫秒
			} catch (InterruptedException e) {
				System.out.println(e);
			}
			executeTransactions(1);
		}
		else if(concurrencyControl.equals("s2pl"))
		{
			executeTransactions(-1);
		}
		else
		...
		if((!concurrencyControl.equals("aria") && !concurrencyControl.equals("calvin") && !concurrencyControl.equals("s2pl")) || (parent.terminalsClose() && reservedOOid == -1))
		{
		...
		for(int i = 0; (i < numTransactions || numTransactions == -1) && !stopRunning; i++)
		{
			...
			*/
			...
			synchronized(lock) 
			{
				...
				{
					ooid = reservedOOid;
					// System.out.println("[Reserved] ooid = " + reservedOOid + ", numTransactions = " + numTransactions + ", i = " + i  + ", csv.numo = " + csv.numo);
				}
				else
				{
					ooid = jTPCC.ooid++;
					// System.out.println("ooid = " + ooid + ", numTransactions = " + numTransactions + ", i = " + i + ", csv.numo = " + csv.numo);
				}
			}
			...
			if(reservedOOid != -1)
			{
				...
				// int randomTime = ThreadLocalRandom.current().nextInt(20, 40);
				// parent.addToAbortTime((long)randomTime);

				// System.out.println("================[Retry] terminalId = " + terminalId + ", retryNum = " + parent.getRetryNum() + "================");
